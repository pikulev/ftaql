...
mod tests {
    ...
    fn test_parse_module() {
        ...
        let (parsed_module, line_count) = parse_module_from_string(ts_code, true, false);

        assert!(parsed_module.is_ok(), "Failed to parse TypeScript code");
        assert_eq!(line_count, 4, "Incorrect line count");
    }
    ...
    fn it_can_be_configured_to_include_comments_in_the_line_count() {
        ...
        let (parsed_module, line_count) = parse_module_from_string(ts_code, true, true);
