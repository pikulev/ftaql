...
mod tests {
    ...
    fn analyze(ts_code: &str) -> HalsteadMetrics {
        let (module_res, _) = parse_module_from_string(ts_code, true, false);
    ...
    fn test_empty_module() {
        ...
        assert_eq!(analyze(ts_code), expected);
    }
    ...
    fn comments_have_no_impact_on_metrics() {
      ...
      "##;
        let commented_code_module = analyze(commented_code);
        let uncommented_code_module = analyze(uncommented_code);
        assert_eq!(commented_code_module, uncommented_code_module);
    }
}
