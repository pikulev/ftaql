...
После запуска можно сразу проверить, какие файлы остаются самыми дорогими от ревизии к ревизии, где растёт связность и когда впервые появились runtime cycles. Это удобно для рефакторинга, CI и анализа истории, когда нужен не одноразовый отчёт, а накопленная база снимков.

Ядро написано на Rust, поэтому повторные прогоны по большим кодовым базам и истории ревизий остаются практичными. На Apple M1 FtaQl анализирует до **3000 файлов в секунду**.
...
| SQLite snapshots | Нормализованные снимки запусков для SQL-запросов и сравнения истории. |
...
Базовый цикл простой: сохранить снимок в SQLite, при необходимости пометить его `revision` и `ref`, а затем запрашивать накопленные данные через SQL.

Сохранить снимок текущей рабочей копии:
...
- `--revision` сохраняет точный идентификатор снимка, обычно SHA коммита
- `--ref` сохраняет человекочитаемую метку ветки, тега или канала, например `main`, `release/1.2` или `nightly`
- вместе они позволяют сравнивать точные снимки и одновременно группировать запуски по ветке или линии релизов

## Что можно спрашивать у истории снимков

После сохранения снимков начинается самое интересное.

Самые проблемные файлы в последнем снимке:
...
FtaQl сохраняет нормализованные снимки проекта в SQLite. Основные таблицы:
...
- обёртка сохраняет снимок и возвращает краткую сводку CLI из `stdout`
